output "incus_network_details" {
  value = {
    ...
    ipv4_domain  = incus_network.network.config["ipv4.domain"]
    ipv4_search  = incus_network.network.config["ipv4.search"]
    ...
    ipv4_dhcp_ranges  = incus_network.network.config["ipv4.dhcp.ranges"]
